...
if(isset($_POST["park_id"]))
{
  ...
  if($pid=="999")
  {
    $result=mysql_query("SELECT * FROM `park_offer`");
  }
  else
  {
    $result=mysql_query("SELECT * FROM `park_offer` WHERE park_id=$pid");
  }
  ...
  if(mysql_num_rows($result)>0)
  {
    ...
    foreach ($datas as $key => $rows)
    {
      ...
      echo "$rows[park_fname],$rows[park_end_datetime],$pname*";
    }
  }
  else
  {
    echo "error116,".constant("error116");
  }
}
else
{
